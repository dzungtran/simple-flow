...
type ExecutionRuntime interface {
	...
	HandleNextNode(stateData []byte) (err error)
...
type FlowExecutor struct {
	...
	partialState []byte // holds the partially completed state
...
// executeNode  executes a node on a flow dag
...
func (fexec *FlowExecutor) createContext() *sdk.Context {
	context := sdk.CreateContext(fexec.id, "", fexec.flowName)
...
// Execute start flow execution
func (fexec *FlowExecutor) Execute(state interface{}) ([]byte, error) {
	var resp []byte
	var gerr error
	...
	if fexec.hasEdge && !fexec.isExecutionFlow {
		return nil, fmt.Errorf("[request `%s`] Failed not an execution flow, DAG data flow need external", fexec.id)
	}
